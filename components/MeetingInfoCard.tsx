...
const MeetingInfoCard: React.FC<MeetingInfoCardProps> = ({ meeting }) => {
  return (
    <div className="bg-[var(--infoBoxBackground)] p-6 rounded-xl shadow-lg w-full">
      {/* Section 1 & 2: Title and Date on the same line */}
      <div className="flex items-baseline mb-3">
        <h3 className="text-3xl font-bold text-[var(--text-primary)] mr-3">{meeting.title}</h3>
        <p className="text-lg italic text-[var(--text-tertiary)]">{meeting.date}</p>
      </div>

      {/* Section 3: Times and Length details */}
      <div className="text-lg text-[var(--text-secondary)] space-y-2 mb-5">
      ...
      {/* Section 4: Notes */}
      <div>
        <h4 className="text-xl font-semibold text-[var(--text-primary)] mb-1.5">Notes:</h4>
        <div className="text-lg text-[var(--text-secondary)] whitespace-pre-wrap bg-[var(--background-primary)] p-4 rounded-md max-h-72 overflow-y-auto">
          {meeting.notes}
        </div>
      </div>
    </div>
